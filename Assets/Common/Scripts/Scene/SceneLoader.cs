...
/// </summary>
public class SceneLoader : SingletonC<SceneLoader>
{
	...
	[SerializeField] private float loadingBarSpeed = 2f;     // Speed of loading bar interpolation
	...
	[SerializeField] StringEvent OnLoadEvent;            // Triggered to request scene load
	[SerializeField] StringEvent OnLoadStartEvent;       // Triggered when loading begins
	[SerializeField] StringEvent OnLoadCompleteEvent;    // Triggered when loading finishes
	...
	{
		isLoading = true;
		OnLoadStartEvent?.RaiseEvent(sceneName);
		...
		isLoading = false;
		OnLoadCompleteEvent?.RaiseEvent(sceneName);
	}
}